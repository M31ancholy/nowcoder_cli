...
func (s *Service) Hunt(company, position, outputDir string) (*Interview, error) {
	...
	if err := s.openNowcoder(); err != nil {
		return nil, err
	}

	if err := s.search(searchQuery); err != nil {
		return nil, err
	}

	if err := s.filterInterview(); err != nil {
		return nil, err
	}

	screenshotPath := filepath.Join(outputDir, fmt.Sprintf("search_%s.png", timestamp))
	if err := s.screenshot(screenshotPath); err != nil {
		return nil, err
	}

	if err := s.clickFirstInterview(); err != nil {
		return nil, err
	}

	interview, err := s.extractInterviewDetail(screenshotPath)
	if err != nil {
		return nil, err
	}
...
func (s *Service) openNowcoder() error {
...
func (s *Service) filterInterview() error {
	if err := chromedp.Run(s.ctx,
		chromedp.Sleep(2*time.Second),
	); err != nil {
		return err
...
func (s *Service) clickFirstInterview() error {
	if err := chromedp.Run(s.ctx,
		chromedp.Sleep(3*time.Second),
	); err != nil {
		return err
...
func (s *Service) extractInterviewDetail(screenshotPath string) (*Interview, error) {
	var title string
	if err := chromedp.Run(s.ctx,
